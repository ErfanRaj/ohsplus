...
export function ProductCard({ product }: { product: ProductRow }) {
  return (
    <Card className="group relative h-full overflow-hidden border-border/70 shadow-none transition-all focus-within:border-primary/60 hover:-translate-y-0.5 hover:border-primary/60">
      <CardContent className="flex h-full flex-col gap-4 p-5">
        ...
        <div className="space-y-2">
          <h3 className="text-base leading-7 font-bold">
            {/* Stretched link: the whole card is clickable while nested controls stay usable. */}
            <Link
              to="/products/$slug"
              params={{ slug: product.slug }}
              className="rounded-sm outline-none after:absolute after:inset-0 after:z-10 after:content-[''] focus-visible:ring-2 focus-visible:ring-ring"
            >
              {product.title}
            </Link>
          </h3>
